% Toy example for tracking a single target using an Unscented Kalman filter
...
ekf_filter = ekf(sys, init);
x_ekf = init.x;     % state
% main loop; iterate over the measurements
for i = 2:size(z,2)
    ekf_filter.prediction();
    ekf_filter.correction(z(:,i));
    x_ekf(:,i) = ekf_filter.x;
end

% UKF
init.kappa_f = 2;
init.kappa_h = 2;
ukf_filter = ukf(sys, init);
x_ukf = init.x;     % state
% main loop; iterate over the measurements
for i = 2:size(z,2)
    ukf_filter.prediction();
    ukf_filter.correction(z(:,i));
    x_ukf(:,i) = ukf_filter.x;
end
...
plot(x_ekf(1,:), x_ekf(2,:), '-.g', 'linewidth', 2)
plot(x_ukf(1,:), x_ukf(2,:), '-k', 'linewidth', 2)
legend('ownship', 'ground truth', 'EKF', 'UKF', 'location', 'best')
grid on, axis auto equal tight
xlabel('$x_1$', 'fontsize', fsize, 'Interpreter','latex')
ylabel('$x_2$', 'fontsize', fsize, 'Interpreter','latex')
set(gca, 'fontsize', fsize)
figuresize(21,21,'cm')
print -opengl -dpng -r600 ukf_example_tracking.png
